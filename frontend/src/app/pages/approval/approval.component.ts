                  ...
                  <div class="w-full md:w-1/2 space-y-1.5 pt-0.5">
                    ...
                    @if (req.target_user) {
                      <div class="flex items-center text-xs">
                        ...
                        <span class="target-user-badge font-bold px-2 py-0.5 rounded border border-blue-300 text-xs" style="color: #0f172a !important; background-color: #dbeafe !important; border-color: #93c5fd !important;">{{ req.target_user }}</span>
                      </div>
                    }
                  ...
                  <div class="border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-200 text-xs">
                            ...
                            <div class="p-2 bg-slate-50/70 border border-slate-200 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                              ...
                              <div class="flex items-start gap-2.5 flex-1 min-w-0">
                                ...
                                <div class="space-y-0.5 min-w-0 flex-1">
                                  ...
                                  @if (item.target_user) {
                                    <div class="target-user-badge text-[10.5px] px-2 py-0.5 rounded border border-blue-300 inline-block font-bold mt-1" style="color: #0f172a !important; background-color: #dbeafe !important; border-color: #93c5fd !important;">
                                      <span style="color: #1e3a8a !important; font-weight: bold !important;">👤 使用者・対象者: </span>
                                      <span style="color: #0f172a !important; font-weight: bold !important;">{{ item.target_user }}</span>
                                    </div>
                                  }
                    ...
                    @if (req.target_user) {
                      <div class="flex p-2 bg-blue-100/60 border-t border-slate-200">
                        ...
                        <span class="font-bold text-slate-950" style="color: #0f172a !important;">{{ req.target_user }}</span>
                      </div>
                    }
...
export class ApprovalComponent {
  ...
  async downloadPDF() {
    ...
    try {
      ...
      // PDF出力時、使用者・対象者バッジおよび内部テキストがhtml2canvasのCSS変数不具合で消えるのを完全防止
      const targetUserElements = clone.querySelectorAll('.target-user-badge, .target-user-badge *');
      targetUserElements.forEach((el) => {
        const htmlEl = el as HTMLElement;
        htmlEl.style.setProperty('color', '#0f172a', 'important');
        htmlEl.style.setProperty('background-color', '#dbeafe', 'important');
        htmlEl.style.setProperty('border-color', '#93c5fd', 'important');
